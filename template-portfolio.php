		...
		<?php if($categorys=="" and $menu=="sorting"){ ?>
			...
			<ul id="filterm">
				...
				<?php foreach ($term_obj as $term) { ?>
					...
					<li><a href="#"  <?php if ($term->slug == 'photos') {?> class="show-subnav filter-item" <?php } else {?>class="close-subnav filter-item" <?php }?>id="<?php echo esc_attr($term->slug); ?>" data-filter=".<?php echo esc_attr($term->slug); ?>"><?php echo esc_attr($term->name); ?></a></li>
	     			...
	     			while ( $loop->have_posts() ) : $loop->the_post();
							...
							if($link=="permalink"){ ?>
									...
									<?php } else if($columns=="3") {  $columnidisotop = "3"; ?>
										<div class="port port-file-3 col-md-4 sorting <?php $post_cats = wp_get_post_terms( get_the_ID(),'portfolio_cat' ); foreach( $post_cats as $category ) { echo sanitize_html_class($category->slug.' ');} ?>" data-category="portfolio">
											<div class="viewport3column">
											    <a href="<?php the_permalink(); ?>">
											        <span class="dark-background-2"></span>
											        <?php 	$src = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), array( 999,999 ), false, '' ); 
											        		echo get_the_post_thumbnail( get_the_ID(), array(480, 240, 'bfi_thumb' => true, 'class' => ' img-zoom'));?>
											    </a>
											    <div class="mt_isotope_text">
											    	<a href="<?php the_permalink(); ?>"><h4 class="widget_h_2"><?php echo get_the_title();  ?></h4></a>
											    	<?php echo balanceTags($portfolio_excerpt); echo "<a href='".get_permalink()."' class='more-link'><span>Read more</span></a>"; ?> 
			    			...
			    			<?php } else { ?>
									...
									<?php  if($columns=="4") { $columnidisotop = "4"; ?>
										<div class="port port-file-3 col-md-3 sorting <?php $post_cats = wp_get_post_terms( get_the_ID(),'portfolio_cat' ); foreach( $post_cats as $category ) { echo sanitize_html_class($category->slug.' ');} ?>" data-category="portfolio">
											<div class="viewport4column">
											    <a data-rel="prettyPhoto[pp_gal]" href="<?php echo esc_url($large_image_lightbox[0]); ?>">
											        <span class="dark-background-2"></span>
											        <?php 
											        	$src = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), array( 999,999 ), false, '' ); 
											        	echo get_the_post_thumbnail( get_the_ID(), array(360, 360, 'bfi_thumb' => true, 'class' => ' img-zoom'));?>
											    </a>
											    <div class="mt_isotope_text">
											    	<a data-rel="prettyPhoto[pp_gal_2]" href="<?php echo esc_url($large_image_lightbox[0]); ?>"><h4 class="widget_h_2"><?php echo get_the_title();  ?></h4></a>
													<?php echo balanceTags($portfolio_excerpt); echo "<a data-rel='prettyPhoto[pp_gal]' href='". esc_url($large_image_lightbox[0]) ."' class='more-link'><span>Read more</span></a>"; ?> <div class="clear"></div>
											    </div>
											</div>
										</div> 
									<?php } else if($columns=="3") {  $columnidisotop = "3"; ?>
